...
void addRoundKey(byte state[][4], word w[]);
...
static byte* invCipher(byte* input,byte* key, int Nr, int Nk)
{
	byte state[4][4];
...
{
	return invCipher(input, key, 10, 4);
}

byte* invAES192(byte* input, byte* key)
{
	return invCipher(input, key, 12, 6);
}

byte* invAES256(byte* input, byte* key)
{
	return invCipher(input, key, 14, 8);
}
